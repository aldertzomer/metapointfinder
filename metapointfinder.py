...
def fasta_to_tsv_two_cols(fa_in: Path, tsv_out: Path):
    ...
    acc = None
    seq_parts = []
    with open(fa_in, "r", encoding="utf-8") as fin, open(
        tsv_out, "w", encoding="utf-8"
    ) as fout:
        for line in fin:
            line = line.rstrip("\n")
            if not line:
...
def write_class_and_gene_summaries(sample_prefix, out_dir: Path, is_prot=True):
    ...
    infile = (
        out_dir / f"{sample_prefix}.{kind}.updated_table_with_scores_and_mutations.tsv"
    )
    ...
    row_classes = sorted(
        set(r["class"] for r in rows if r.get("class") and r["class"] != "class")
    )
    # include DB classes as well (print zero rows too)
    classes = sorted(set(row_classes) | set(ALL_CLASSES))
    genes = sorted(
        set(r["gene"] for r in rows if r.get("gene") and r["gene"] != "gene")
    )
        ...
        for c in classes:
            for g in genes:
                WT = sum(
                    1
                    for r in rows
                    if r["class"] == c and r["gene"] == g and r["Status"] == "Wildtype"
                )
                R = sum(
                    1
                    for r in rows
                    if r["class"] == c and r["gene"] == g and r["Status"] == "Resistant"
                )
                NA = sum(
                    1
                    for r in rows
                    if r["class"] == c and r["gene"] == g and r["Status"] == "Unknown"
                )
...
def main():
    ...
    try:
        sh(
            "R -e \"stopifnot(3 == length(find.package(c('parallel','Biostrings', 'pwalign'))))\" --slave"
        )
    ...
    if fastqfile.endswith(".gz"):
        ...
        with gzip.open(input_path, "rb") as fin, open(
            output / f"{sample}.fastq", "wb"
        ) as fout:
    ...
    if not exists("AMRProt.fa"):
        ...
        sh(
            f"wget -nv -c {base}/AMRProt-mutation.tsv 1>> databaseprep.log 2>> databaseprep.error"
        )
        sh(f"wget -nv -c {base}/AMRProt.fa 1>> databaseprep.log 2>> databaseprep.error")
    if not exists("AMRProt-mutation_underscore.tsv"):
        write_text(
            "AMRProt-mutation_underscore.tsv",
            read_text("AMRProt-mutation.tsv").replace("/", "_"),
        )

    # Combined protein changes
    if not exists("AMRProt-mutation_underscore_combined.tsv"):
        print("Prepping AMRProt mutation database")
        combined = parse_amrprot_mutation_underscore_combined(
            "AMRProt-mutation_underscore.tsv"
        )
        with open(
            "AMRProt-mutation_underscore_combined.tsv", "w", encoding="utf-8"
        ) as out:
            for (cls, acc), changes in sorted(
                combined.items(), key=lambda t: (t[0][0], t[0][1])
            ):
    ...
    if not exists("AMRProt.dmnd"):
        ...
        sh(
            "diamond makedb --in AMRProt_mutation.fa -d AMRProt 1>> databaseprep.log 2>> databaseprep.error"
        )
    ...
    amr_map = {}
    with open(
        f"{database}/AMRProt-mutation_underscore_combined.tsv", encoding="utf-8"
    ) as f:
        for line in f:
            parts = line.rstrip("\n").split("\t")
            if len(parts) >= 3:
    ...
    sh(
        f'Rscript "{script_dir}/prot_score_mutations.R" "{sample}.prot.input.tsv" {threads} 1>> {sample}.log 2>> {sample}.error'
    )
    shutil.copyfile(
        "updated_table_with_scores_and_mutations.tsv",
        f"{sample}.prot.updated_table_with_scores_and_mutations.tsv",
    )
    # os.remove("input.tsv")
    ...
    with gzip.open(f"{sample}.frag.gz", "rt", encoding="utf-8") as ffrag, open(
        out_path, "a", encoding="utf-8"
    ) as fout:
        for line in ffrag:
            cols = line.rstrip("\n").split("\t")
            ...
            for cls, ref, chg in dna_map.get(accession, []):
                ...
                fout.write(
                    "\t".join([cls, accession, read_id, ref, target, chg]) + "\n"
                )
    ...
    sh(
        f'Rscript "{script_dir}/dna_score_mutations.R" "{sample}.dna.input.tsv" {threads} 1>> {sample}.log 2>> {sample}.error'
    )
    shutil.copyfile(
        "updated_table_with_scores_and_mutations.tsv",
        f"{sample}.dna.updated_table_with_scores_and_mutations.tsv",
    )
    os.remove("updated_table_with_scores_and_mutations.tsv")
    ...
    if exists(f"{sample}.prot.updated_table_with_scores_and_mutations.tsv"):
        print(
            f"Protein read classification output (WT/R) can be found in the file {output}/{sample}.prot.updated_table_with_scores_and_mutations.tsv"
        )
    else:
    ...
    if exists(f"{sample}.class.prot.summary.txt"):
        print(
            f"Class protein summary of classification output (WT/R) can be found in the file {output}/{sample}.class.prot.summary.txt"
        )
        print(
            f"Gene protein summary of classification output (WT/R) can be found in the file {output}/{sample}.gene.prot.summary.txt"
        )
        print()
    else:
    ...
    if exists(f"{sample}.dna.updated_table_with_scores_and_mutations.tsv"):
        print(
            f"DNA read classification output (WT/R) can be found in the file {output}/{sample}.dna.updated_table_with_scores_and_mutations.tsv"
        )
    else:
    ...
    if exists(f"{sample}.class.dna.summary.txt"):
        print(
            f"Class DNA summary of classification output (WT/R) can be found in the file {output}/{sample}.class.dna.summary.txt"
        )
        print(
            f"Gene DNA summary of classification output (WT/R) can be found in the file {output}/{sample}.gene.dna.summary.txt"
        )
        print()
        sys.exit(0)
    else:
