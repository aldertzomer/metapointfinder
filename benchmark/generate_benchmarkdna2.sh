...
# create a database of reference genomes that are used to pad the sequence with. The reviewer was not happy with random DNA being used for the padding around the gene or protein.
bash build_context_genome_db_dna.sh

#pad the sequences to 6 kb to simulate 6 kb pseudogenomes using real genomes. wgsim does not want to generate reads that are close to the length of the gene and crashes.
python make_genomic_context_benchmark_local.py benchmarkdna/mutated_underscore.fna benchmarkdna/mutated_underscore.6000.fasta \
        --db benchmarkdna/context_genomes/context_genomes.fna \
        --genomes benchmarkdna/context_genomes/context_genomes.fna \
        --length 6000 \
        --num-threads 16 \
        --metadata benchmarkdna/local_context_metadata.tsv \
        --cache benchmarkdna/local_context_cache.json
...
for r in 0.00 0.01 0.02 0.03 0.05 0.1 0.2 0.3 ; do
        echo "python $tool --input benchmarkdna/100.$r.fastq --db $db --output benchmarkdna/mpf.100.$r --identity 50 --threads 8"
        echo "python $tool --input benchmarkdna/200.$r.fastq --db $db --output benchmarkdna/mpf.200.$r --identity 50 --threads 8"
        echo "python $tool --input benchmarkdna/1000.$r.fastq --db $db --output benchmarkdna/mpf.1000.$r --identity 50 --threads 8"
	echo "python $tool --input benchmarkdna/5000.$r.fastq --db $db --output benchmarkdna/mpf.5000.$r --identity 50 --threads 8"
done |parallel -j 16
...
find benchmarkdna |grep dna.updated_table_with_scores_and_mutations.tsv$ |while read output ; do
    ...
    sample=`echo $output|cut -f 2 -d "/" | sed 's/mpf.//'` ;  cat $output |cut -f 3 |grep WT |sort|uniq >$output.wtmappedreadlist ; cat benchmarkdna/$sample.fastq |grep ^"@" |sed 's/^@//' |grep WT >$output.wtallreadlist ; cat $output.wtmappedreadlist $output.wtallreadlist |sort|uniq -c |grep " 1 " |wc -l |tr "\n" ","
    sample=`echo $output|cut -f 2 -d "/" | sed 's/mpf.//'` ;  cat $output |cut -f 3 |grep RES |sort|uniq >$output.resmappedreadlist ; cat benchmarkdna/$sample.fastq |grep ^"@" |sed 's/^@//' |grep RES >$output.resallreadlist ; cat $output.resmappedreadlist $output.resallreadlist |sort|uniq -c |grep " 1 " |wc -l
