...
calculate_mutation_score <- function(reference, target, changes_str) {
  ...
  # Create a data frame to store position, reference amino acid, and target amino acid
  ...
  for (change in change_pairs) {
    # Extract position and reference/target amino acids from each change (e.g., A51V -> Position 51, A -> reference, V -> target)
    ...
    reference_aa <- substr(change, 1, 1)
    target_aa <- substr(change, nchar(change), nchar(change))
  ...
  # Convert sequences to AAStringSet objects with proper error handling
  tryCatch({
    ...
    for (i in 1:nrow(change_data)) {
      ...
      target_aa <- change_data$TargetAA[i]
      
      # Ensure position is within bounds of the target sequence
      if (pos <= length(target_chars)) {
        # Check if the target amino acid at the position matches the expected change
        if (target_chars[pos] == target_aa) {
          score <- score + 1
          detected_mutations <- c(detected_mutations, 
                                paste0(ref_aa, pos, target_aa))
        }
      }
    }
