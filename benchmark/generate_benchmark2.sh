...
bash build_context_genome_db.sh

# pad the sequences to 6 kb to simulate 6 kb pseudogenomes using real genomes. wgsim does not want to generate reads that are close to the length of the gene and crashes. the padding is required.
#for cut in Eacica Eagrtu Ebacsu Ebraja Ecaucr Echltr Echlre Ecloab Eerwct Eecoli Ebacst Ehaein Ehalsa Eklepn Elacdl Emyctu Eneigo Epseae Erhile Erhosh Esalty Eserma Erhime Estaau Estrpn Estrco Esynco Etheth Evibch Eyeren  ; do
for cut in Eecoli ; do
    #echo "python pad_to_10kb.py benchmark/$cut.fasta benchmark/$cut.6000.fasta --length 6000 --trim center --seed 42"
    echo "python make_genomic_context_benchmark_local.py benchmark/$cut.fasta benchmark/$cut.6000.fasta \
        --db benchmark/context_genomes/context_genomes.fna \
        --genomes benchmark/context_genomes/context_genomes.fna \
        --length 6000 \
        --num-threads 16 \
        --metadata benchmark/local_context_metadata.tsv \
        --cache benchmark/local_context_cache.json"
done | parallel -j 1
...
for cut in Eecoli ; do
    for r in 0.00 0.01 0.02 0.03 0.05 0.1 0.2 0.3; do
	echo "python $tool --input benchmark/$cut.100.$r.fastq --db $db --output benchmark/mpf.$cut.100.$r --identity 50 --threads 2 "
	echo "python $tool --input benchmark/$cut.200.$r.fastq --db $db --output benchmark/mpf.$cut.200.$r --identity 50 --threads 2 "
	echo "python $tool --input benchmark/$cut.1000.$r.fastq --db $db --output benchmark/mpf.$cut.1000.$r --identity 50 --threads 2"
	echo "python $tool --input benchmark/$cut.5000.$r.fastq --db $db --output benchmark/mpf.$cut.5000.$r --identity 50 --threads 2"
    done
done |parallel -j 64
...
echo "file,WT,RES,FP,FN,WT_UNK,RES_UNK" > benchmark/final_scores.csv
...
echo "file,WT,RES,FP,FN,WT_UNK,RES_UNK,WT_unmapped,RES_unmapped" > benchmark/final_scores2.csv
find benchmark |grep prot.updated_table_with_scores_and_mutations.tsv$ |while read output ; do
    echo $output |tr "\n" ","
