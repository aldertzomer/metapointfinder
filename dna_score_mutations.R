...
# Uses simple leading-offset model and calls deletions & substitutions
calculate_mutation_score <- function(reference, target, changes_str) {
  ...
  for (change in change_pairs) {
    ...
    reference_DNA <- toupper(substr(change, 1, 1))
    target_DNA    <- toupper(substr(change, nchar(change), nchar(change)))  # may be "-"
  ...
  tryCatch({
    ...
    for (i in 1:nrow(change_data)) {
      ...
      base_in_read <- target_chars[aln_idx]
      base_in_ref  <- ref_chars[aln_idx]

      # Deletion call: expected deletion (TargetDNA == "-") and read shows a gap at this ref column
      if (target_DNA == "-") {
        if (base_in_read == "-") {
          score <- score + 1
      ...
      # Treat read '-' or 'N' as uninformative (neither WT nor R)
      if (base_in_read %in% c("-", "N")) next

      # Resistance substitution?
      if (base_in_read == target_DNA) {
        score <- score + 1
        detected_mutations <- c(detected_mutations, paste0(ref_DNA, pos, target_DNA))
      # Explicit WT confirmation (ensure the reference at this column is indeed the ref base)
      } else if (base_in_ref == ref_DNA && base_in_read == ref_DNA) {
